...
@section('content')
    <div class="space-y-4">
        ...
        <form action="{{ route('admin-article-update', ['id' => $article_id]) }}" method="post">
            @csrf
                    ...
                    <div class="grid grid-cols-12 gap-4 rounded-md border bg-white p-4">
                        ...
                        <div class="col-span-12">
                            ...
                            <textarea name="excerpt" id="excerpt" rows="3"
                                class="mt-1 block w-full rounded-md border-0 py-1.5 text-c0-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-c1-600 sm:text-sm">{{ old('excerpt') ?? $article->excerpt }}</textarea>
                        </div>
                        <div class="col-span-12">
                            <label for="content" class="block text-sm font-medium leading-6 text-c0-900">Konten <span
                                    class="text-red-500">*</span></label>
                            <textarea name="content" id="content" rows="12"
                                class="trumbowyg-editor mt-1 block w-full rounded-md border-0 py-1.5 text-c0-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-c1-600 sm:text-sm">{{ old('content') ?? $article->content }}</textarea>
                            ...
                            <div class="space-y-4 p-4">
                                <div>
                                    ...
                                    <select name="status" id="status"
                                        class="mt-1 block w-full rounded-md border-0 py-1.5 text-c0-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-inset focus:ring-c1-600 sm:text-sm">
                                        <option value="draft"
                                            {{ (old('status') ?? $article->status) == 'draft' ? 'selected' : '' }}>Draft
                                        </option>
